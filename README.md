...
very first run needs nothing but the two commands below. `git clone
--recursive` or `git submodule update --init` fetches the library. `npm
install` only runs the first time (the scripts do it automatically).
