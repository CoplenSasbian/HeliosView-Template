...
//
// Wire-up is intentionally thin: create the AppContext (UI loop + thread pool,
// both wrapped by the HeliosView library), create the MainWindow (a
// WebViewWindow subclass that owns the native <-> JS bridge and frontend
// loading), then run the UI loop. The window is destroyed before the context,
// so no binding or pending task can outlive the loop.
//
...
int main()
{
    // 1) The context: UI loop + background thread pool.
    AppContext ctx;
